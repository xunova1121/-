...
/**
 * 焦段。景别是**焦距 + 距离**算出来的，不是一个形容词 ——
 * "中景"里 35mm 站 2 米和 85mm 站 5 米是完全不同的两张画。
 *
 * 括注写的是**画面上看得出来的那件事**，不是参数：选的人多半不懂
 * 毫米数意味着什么，但一定知道自己想不想要"背景压缩"。
 */
const LENSES = [
  ['', '不指定'],
  ['24', '24 广角'],
      ...
      h('div', { class: 'ed-group' },
        ...
        field('焦段',
          chips(LENSES.map(([, label]) => label),
            LENSES.find(([v]) => v === String(s.lens || ''))?.[1] || '不指定',
