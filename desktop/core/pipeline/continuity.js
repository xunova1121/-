 ...
 * 收的有方舟、可灵、Vidu、秘塔；海螺官方口、百炼、Sora 不收。
 *（别把这几个名字写死在判断里 —— 唯一的事实来源是 catalog 里的
 *  videoDefaults.endFrame，这段注释只是举例，改厂商时以目录为准。）
 *
 * 用不收的那几家时，那条路整个是空的：我们会说一句"这家不收末帧"，
...
export function continuityLines(shot, { prev, next, link, brief: short = false } = {}) {
  ...
  if (rel === 'continuous' && prev) {
     ...
     * 原因是**每一段都是独立生成的**：模型只看到"这一镜演什么、多长"，
     * 看不到上一段人是用什么速度走的，于是自己挑一个 —— 两段挑到不一样的
     * 概率相当高，而观众对速度突变极其敏感（比对人脸漂移敏感得多）。
     *
     * 首帧接住了也救不了这一条：首帧只定住第一格，速度是后面几秒的事。
     */
    lines.push('运动速度和上一镜保持一致，不要突然加快或放慢');
