 ...
 * 是**一模一样的毛病**：焦段是行话，模型对它的理解未必和你一样，
 * 各家理解也不一致，而它要和几百字的描述抢注意力。
 *
 * 「特写」那次的解法是配一句能验的话（"画面只到肩膀以上"）。这里照办：
 * 焦段真正决定的是**背景被压缩多少、透视有多夸张**——那两样在成图上
 * 一眼就看得出来，比"85mm"三个字硬得多。
 *
 * ⚠ 没选焦段的镜头**一个字都不说**。
 * 补一句默认的 35mm 说明，等于替用户做了一个他没做过的决定，
 * 而且是一个听起来很具体的谎 —— 这个文件里已经为"没摆地标就别编"
 * 单独写过一段了，同一条道理。
 */
const LENS_HINTS = [
...
/** 这个焦段在画面上长什么样。没给焦段就返回空 —— 不替人编一个 */
export function lensHint(lensMm) {
  const f = Number(lensMm);
  if (!Number.isFinite(f) || f <= 0) return '';
  return LENS_HINTS.find((x) => f <= x.max).text;
}
