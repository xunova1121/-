...
- **画面里看得见的人，全部列进 characters** —— 哪怕这一镜的主体是道具、
  是手机屏幕、是一只手。特写不等于没有人：一个"手机屏幕特写"，画面里
  多半还有举着手机的那个人的脸和肩膀，她就得列进去。
  ⚠ 漏列的代价不是"少一行字"，是**这一镜不会带她的参考图**：
  出来的人换了张脸、换了身衣服，而前后两镜是同一场戏里连着的。
  真正一个人都没有的空镜（桌上一支钢笔、窗外的雨）才给空数组；
- duration 取 3~6 秒。
...
async function analyzeScriptRaw(projectId, {
   ...
   *
   * 每一镜十来个字段，description / motion 还是自由文本 —— 代码里原来那句
   *「12 镜 ≈ 2500 token」是**紧凑写法**的估算，模型写细一点翻一倍很正常。
   * 而估算错了的代价全落在用户身上：跑几分钟、花了钱、拿到半截。
   *
  ...
  for (;;) {
    try {
      ...
      /**
       * ⚠ 只在**输出被截断**这一种上缩批。401、模型不存在、内容审核
       * 缩了也是再错一次，还多花一次钱。
       *
       * 缩不动了（只剩一场）就老实抛出去 —— 一场戏再长也不能不拆，
       * 那种情况真正该做的是回大纲把那一场拆成两场，而那是人的判断。
       */
...
const RUN_ORDER = [
  /**
   * ⚠ 大纲排在**设定集前面**：设定集要照着大纲的场次和角色名单出，
   * 名字才是一套的。理由见 store.js 里 STAGES 上面那段。
   *
