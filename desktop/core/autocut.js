...
/**
 * 富余在这个数以内时，**从头上切**而不是从尾上切。
 *
 * 2 秒是照着厂商档位定的：秘塔 5/10/15、海螺 6/10、方舟 5/10 ——
 * 分镜写 3~5 秒时，被顶上去的富余基本都在 2 秒以内，那就是"厂商凑档"
 * 多出来的部分，从头上扔掉最安全。
 *
 * 再多就不是凑档了，是分镜时长和实际内容对不上（比如写 6 秒顶成 10 秒）。
 * 那种情况从头上砍六秒会连动作开头一起砍掉 —— 没有好答案，
 * 该用 keep 策略保留整段。
 */
export const HEAD_ANCHOR = 2.0;
...
export function pickWindow(hashes, total, want, { step = STEP, hamming, dead: given = null, ...opts } = {}) {
   ...
   * 把一些好的画面给截了"。
   *
   * 形状是这样的：分镜写 4 秒，而厂商只出固定档（秘塔 5/10/15），
   * 给回来一段 5 秒。原来的做法是切 [0, 4] —— 扔掉最后一秒。
   * 而模型是按"这一段演完"编排节奏的：**起手慢、收在最后**。
   * 那一秒恰恰是动作收尾，也就是这一镜唯一的看点。
   *
   * 同样是砍一秒，从头上砍砍掉的是起势，从尾上砍砍掉的是收尾。
   * 前者观众看不出来，后者一眼就看出"没演完就切了"。
   *
   * 只在富余不多时这么做（HEAD_ANCHOR）：厂商把 6 秒顶成 10 秒那种
   * 大富余，从头上砍六秒会把动作的开头也砍掉 —— 那种情况没有好答案，
   * 该走 keep 策略保留整段，而不是在这儿硬挑。
   */
  const spare = total - length - start;
  if (spare > 0 && spare <= HEAD_ANCHOR) start += spare;
