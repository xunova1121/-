...
const DEFAULTS = {
  ...
  refMode: 'auto',
  /**
   * 出分镜图时带哪些参考图。见 consistency.refPlan。
   *
   *   auto（默认）只发**你自己传上去的那些图**，模型不换。
   *               传一张照片是一句毫不含糊的"我要这张脸"；
   *               而模型出的设定图只是个近似，没必要为它冒换模型的险。
   *   all         所有设定图都当参考发，模型不换。
   *   edit        换成图生图（编辑）模型再发图。构图会被带跑，慎用。
   *   off         一张都不发（纯文字 + 稳定种子），也就是这个开关加进来之前的行为。
   *
   * ⚠ 这一项修的是一个实打实的洞：在它之前，`useEditModelForShots`
   * 把"发不发参考图"和"换不换模型"捆成了一个开关，而它默认关着 ——
   * 于是用户传了照片上去，出分镜图时**一张都不发**，影响是零。
   * 用户的原话是"传本地图，出的分镜和自传图没有任何关系"。
   */
  refMode: 'auto',
  /**
   * 视频提示词的详略。
  ...
  jsonModeOff: false,
  /**
   * 长生成（拆分镜、出大纲这些）多久没有新内容算死，毫秒。
   *
   * 不是"总共能跑多久"—— 一直在吐字就一直等，这里管的是**静默多久**。
   *
   * 默认 240 秒，比第一版的 90 秒宽得多，因为思考型模型（Claude 的
   * extended thinking、o 系列）在"想"的时候一个 token 都不吐，
   * 而中转站不会把思考过程转成 SSE，那几分钟在我们看来就是没动静。
   * 误杀一次正在思考的调用，代价是那次的钱白花、而且人会以为这条路不通。
   */
  longIdleMs: 240000,
  theme: 'dark',
  port: 5178,
