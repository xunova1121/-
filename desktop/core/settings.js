...
const DEFAULTS = {
   ...
   *          不依赖厂商支持任何东西 —— 每一家 i2v 都收首帧图。
   *          接缝在像素上就是连的。默认。
   *   lock   只用老办法：把这一镜那张图锁成上一段的末帧。
   *          依赖厂商收末帧 —— 只有方舟、可灵、Vidu 收，
   *          海螺、秘塔、百炼、Sora 都不收，用那几家时这条等于没做。
   *   off    什么都不做，只靠提示词衔接。
   *
   * 默认 tail 是因为它是唯一**在所有厂商上都成立**的那条。
   * 两条不冲突：收末帧的厂商上，tail 模式仍然会顺带锁末帧，双保险。
   */
  /**
