...
export const STYLE_PRESETS = [
  ...
  },
  {
    ...
    hint: '青绿设色、云海雾气、暖金天光',
    /**
     * 这一条按参照图重写过**两遍**，第二遍推翻了第一遍。
     *
     * 第一遍写的是真正的绢本工笔：「细笔勾线填色、绢本设色质感」。术语没错，
     * 但它指向的是另一种东西 —— 平面、装饰性、线描填色，像一幅纹样。
     * 而大家说「古风工笔」时心里想的那张图，其实是**新中式国潮插画**：
     *
     *   柔和渐变上色，没有粗黑轮廓线（这一条最关键，"勾线"两个字会直接毁掉它）
     *   青绿 + 米白的低饱和配色，整体压成高级灰
     *   云海把中景切断，远山三重渐隐 —— 纵深全靠这个
     *   天际一大片暖金光晕，人物压成偏暗的剪影
     *
     * 教训是：写风格锚不能写**门类名**，要写**画面特征**。「工笔」是门类，
     * 模型照着门类去画，给回来的是教科书里那种工笔；把渐变、无轮廓、雾气、
     * 光晕这些看得见的特征逐条写出来，才画得到你想要的那张。
     *
     * 顺带一句：构图特征（背影、人物很小、一轮巨日）**故意没写进去**。
     * 风格锚会拼在每一镜前面，把构图写死等于全片二十个镜头全是背影。
     */
    anchor:
      '新中式国潮古风插画，柔和渐变上色，没有粗黑描边，青绿与米白的低饱和配色，'
      + '云海雾气弥漫，远山三重渐隐，天际大片暖金色光晕，飞檐楼阁与松枫点缀在雾中，'
      + '细腻笔触，大量留白，电影感构图',
    negative:
      '粗黑描边, 硬轮廓线, 高饱和, 荧光色, 浓重阴影, 厚涂油画, 漫画网点, '
      + '3D渲染, 塑料质感, 满构图无留白, 现代元素, 西方油画笔触',
    palette: '青绿、米白、暖金',
    swatch: { from: '#F3E6CE', to: '#3E6B63', accent: '#E8C27A', texture: 'fine' },
    art: {
      sky: ['#F3E6CE', '#E3CBA2'],
      hills: ['#9EC6C0', '#6E9A93', '#3E6B63'],
      water: '#DCE7E0',
      glow: '#E8C27A',
      ink: '#33534E',
      // 渐变、无轮廓 —— 用勾线那套画示意图，等于在卡片上就把风格说错了
      mode: 'soft'
    }
  },
  {
