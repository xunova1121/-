...
function shapeHint(ak) {
  ...
  /**
   * 30 位、不是 LTAI 开头 —— 这几乎可以断定是**把 Secret 填到 ID 那一栏了**。
   *
   * 阿里云这两样的长度是固定的、而且不一样：
   *   AccessKey ID      24 位，LTAI 开头
   *   AccessKey Secret  30 位，纯随机
   * 所以"30 位"这一条本身就是签名。上一版只会含糊地说"八成粘错了东西
   *（RAM 用户名？登录名？还是把 Secret 粘到 ID 这一栏了？）"—— 三个猜测
   * 并列，等于让用户自己去试。而长度已经把答案写在脸上了。
   *
   * ⚠ 还要提醒**两栏一起重填**：这两个框是"留空表示不改"的，
   * 只改 ID 那一栏的话，Secret 那一栏还留着上次填错的东西。
   */
  if (k.length === 30) {
    return '⚠ **30 位、不是 LTAI 开头 —— 这就是一把 AccessKey Secret。**'
      + '阿里云的 ID 是 24 位、LTAI 开头，Secret 是 30 位 —— 你把两栏填反了。'
      + '回控制台把 ID 那串（LTAI…）填到「AccessKey ID」，Secret 填到下面那栏。'
      + '⚠ **两栏都要重填**：这两个框是"留空表示不改"，只改一栏的话另一栏还留着填错的那份。';
  }
  if (!/^LTAI/i.test(k)) {
