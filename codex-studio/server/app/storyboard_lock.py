...
def build_snapshot(db: sqlite3.Connection, project_id: str, episode: int) -> tuple[dict[str, Any], list[str]]:
    ...
    bible_rows = db.execute(
        "SELECT entity_type,entity_key,name,version,state,fingerprint FROM bible_entities b WHERE project_id=? "
    ...
    bible = [{key: row[key] for key in row.keys()} for row in bible_rows]
    contracts = [{
...
def status(db: sqlite3.Connection, project_id: str, episode: int) -> dict[str, Any]:
    ...
    if not row or row["status"] != "locked":
        return {
            ...
            "locked_at": None, "shot_count": current["shot_count"], "ready_to_lock": not issues,
    ...
    return {
        ...
        "ready_to_lock": not issues, "issues": issues + (["剧本、设定、连续性或分镜已在锁定后修改"] if stale else []),
