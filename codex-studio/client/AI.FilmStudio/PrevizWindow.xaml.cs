...
public partial class PrevizWindow : Window
{
    ...
    private void Draw()
    {
        if (!IsLoaded || Stage.ActualWidth < 100) return;
        var cx = Stage.ActualWidth / 2;
        var cy = Stage.ActualHeight / 2;
        double Scale(double n) => n * 45;
        var camX = cx + Scale(Read(CameraX));
        var camY = cy + Scale(Read(CameraY));
        var subX = cx + Scale(Read(SubjectX));
        var subY = cy + Scale(Read(SubjectY));
        Canvas.SetLeft(CameraDot, camX - 12); Canvas.SetTop(CameraDot, camY - 12);
        Canvas.SetLeft(CameraLabel, camX + 14); Canvas.SetTop(CameraLabel, camY - 10);
        Canvas.SetLeft(SubjectDot, subX - 15); Canvas.SetTop(SubjectDot, subY - 15);
        Canvas.SetLeft(SubjectLabel, subX + 18); Canvas.SetTop(SubjectLabel, subY - 10);
        AxisLine.X1 = camX; AxisLine.Y1 = camY; AxisLine.X2 = cx + Scale(Read(TargetX)); AxisLine.Y2 = cy + Scale(Read(TargetY));
        ViewLeft.X1 = camX; ViewLeft.Y1 = camY; ViewLeft.X2 = AxisLine.X2 - 130; ViewLeft.Y2 = AxisLine.Y2;
        ViewRight.X1 = camX; ViewRight.Y1 = camY; ViewRight.X2 = AxisLine.X2 + 130; ViewRight.Y2 = AxisLine.Y2;
    }
